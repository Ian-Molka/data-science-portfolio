...
contextual_patterns = [
    "cargo spill", "jackknife", "overturned", "tanker",
    "box truck", "big rig", "commercial vehicle"
]
...
df['vehicle_type_cmv'] = (
    df['vehicle_body_description'].isin(cmv_types).astype(int)
    if 'vehicle_body_description' in df.columns else 0
)
...
def model_and_make_is_cmv(row):
    ...
    return (
        ...
        any(keyword.replace(" ", "") in model for keyword in cmv_model_keywords)
    )
...
# === Step 12: Optional OBJECTID using Parking Lot logic (if present) ===
...
if 'Parking Lot' in df.columns:
    df['OBJECTID'] = df['Parking Lot'].apply(extract_objectid)
else:
    print("\nℹ️ No 'Parking Lot' column in this file; skipping OBJECTID update.")
...
print(
    df['cmv_involved']
    .value_counts()
    .rename(index={True: 'True (CMV Involved)', False: 'False (Not Involved)'})
)
print(f"\n✅ Updated file saved with corrected 'cmv_involved' (OBJECTID only if Parking Lot existed):\n{output_path}")
